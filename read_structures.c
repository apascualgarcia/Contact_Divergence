...
int Read_PDB_list(struct protein *prots, int N_pdb, char *PDB_PATH,
		  int CM_FILE, char CONT_TYPE, float CONT_THR, int IJ_MIN)
{
  int i; char FILE_TMP[NCHAR];
  strcpy(FILE_TMP, "pdb.tmp");
  for(i=0; i<N_pdb; i++){
    char filename[300], command[300], chain=prots[i].chain;
...
{
  FILE *file_in;
  int ip, i, L, Nc, nc, nc_max=0, Np=0, k, i1, i2;
  short **Cont_map, *n_cont;
  ...
  if(Np!=N_prot){
    ...
    for(ip=0; ip<N_prot; ip++)free(names[ip]); free(names);
    return(0);
  }
